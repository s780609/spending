    ...
    <main className="mx-auto w-full max-w-3xl flex-1 px-4 py-8 pb-24 sm:pb-8">
      ...
      <div className="mt-6 rounded-2xl bg-white p-4 shadow-sm ring-1 ring-gray-950/10">
        ...
        <form
          ...
          className="mt-4 grid grid-cols-2 gap-3 border-t border-gray-950/5 pt-4 sm:grid-cols-[1fr_1fr_auto]"
        >
          <label className="col-span-2 flex flex-col gap-1 text-xs text-gray-500 sm:col-span-1">
            ...
            <input
              ...
              className="rounded-lg px-3 py-2 text-base ring-1 ring-inset ring-gray-950/10 focus:outline-none focus:ring-2 focus:ring-gray-950 sm:text-sm"
            />
          </label>
          <label className="col-span-2 flex flex-col gap-1 text-xs text-gray-500 sm:col-span-1">
            校正目前里程（留空＝不更動）
          ...
          <button
            type="submit"
            className="col-span-2 self-end rounded-full bg-gray-950 px-4 py-2.5 text-base font-medium text-white hover:bg-gray-800 sm:col-span-1 sm:py-2 sm:text-sm"
          >
            儲存
          </button>
        </form>
      ...
      <form
        ...
        className="mt-4 grid grid-cols-2 gap-3 rounded-2xl bg-white p-4 shadow-sm ring-1 ring-gray-950/10 sm:grid-cols-[1fr_10rem_auto]"
      >
        <select
          name="itemKey"
        ...
        <input
          ...
          name="note"
          placeholder="備註（選填）"
          className="order-4 col-span-2 rounded-lg px-3 py-2 text-base ring-1 ring-inset ring-gray-950/10 focus:outline-none focus:ring-2 focus:ring-gray-950 sm:order-none sm:col-span-1 sm:text-sm"
        />
        <button
          type="submit"
              ...
              .map((s) => {
                ...
                const badge = due
                  ...
                  : soon
                    ...
                    : {
                        text: "正常",
                        cls: "bg-gray-950/[0.04] text-gray-500 ring-gray-950/5",
                      };
                return (
                  <li
                    key={s.key}
                  ...
                  >
                    ...
                    {s.note && (
                      <p className="mt-1.5 text-xs text-gray-400">{s.note}</p>
                    )}
                  </li>
                );
              })}
      ...
      <section className="mt-8">
        <h2 className="text-sm font-semibold text-gray-600">保養紀錄</h2>
        {records.length === 0 ? (
          <p className="mt-3 text-center text-sm leading-7 text-gray-600">
            還沒有任何保養紀錄，先記一筆換機油吧。
          </p>
        ) : (
          <ul className="mt-3 space-y-2">
